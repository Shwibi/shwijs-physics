...
class Angle {
	...
	static IsValidUnit(unit = "deg", callback = (err = new shwijs.Err()) => {}) {
		if (!Angle.all.includes(unit?.toLowerCase()))
			return callback(
				new shwijs.Err("Unit must be in deg or rad!", "INV_UNIT", {
					unit,
					dont_log: true,
				})
			);
	...
	rotate(angle = new Angle()) {
		if (angle.unit == this.unit) this.value += angle.value;
			...
			if (Angle.IsRad(this.unit)) {
				...
				return new shwijs.Err("Unknown unit! Cannot add!", "INV_UNIT", {
	...
	get toRadian() {
		...
		if (Angle.IsDeg(this.unit)) {
			...
			return new shwijs.Err("Invalid angle unit, cannot convert!", "INV_UNIT", {
				unit: this.unit,
			});
	}
